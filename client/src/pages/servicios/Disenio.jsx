...
const EntregableItem = ({ number, title, children, isLast, itemRef, nextItemRef }) => {
  ...
  return (
    <div ref={itemRef} className="entregable-item-wrapper">
      <div className="history-item">
        <div className="history-item-header">
          <motion.span
            className="entregable-number bg-yellow-2"
...
const Disenio = () => {
  ...
  // Delay entre letras: 0.0167s (60 caracteres por segundo)
  const letterDelay = 0.0167;
  
  // Dividir el texto en palabras, preservando espacios
  const words = React.useMemo(() => animatedText.split(/(\s+)/), [animatedText]);
  
  // Calcular los índices de letras de forma eficiente usando useMemo
  const letterIndices = React.useMemo(() => {
    let index = 0;
    return words.map(word => {
      const startIndex = index;
      index += word.length;
      return { word, startIndex, endIndex: index };
    });
  }, [words]);

  // Controlar la animación letra por letra
  ...
  React.useEffect(() => {
    if (baseOpacity >= 0.9) {
      ...
      // para que se anime letra por letra
      // Después de un tiempo, marcar como animado para que todas las letras
      // se mantengan visibles sin delay en futuros cambios
      if (!hasAnimated) {
        const totalLetters = animatedText.length;
        const totalAnimationTime = (totalLetters * letterDelay + 0.3) * 1000;
  ...
  }, [baseOpacity, hasAnimated, animatedText, letterDelay]);
    ...
    <>
      ...
      <div ref={animatedTextContainerRef} className="full-container">
        <div className="container animated-text-container">
          ...
          >
            {letterIndices.map(({ word, startIndex }, wordIndex) => {
