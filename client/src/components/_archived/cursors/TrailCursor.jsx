...
const TrailCursor = () => {
  ...
  // Verificar si estamos en una ruta de servicios o en Nosotros
  const isServiciosRoute = location.pathname.startsWith('/servicios');
  const isNosotrosRoute = location.pathname === '/nosotros';
  const shouldActivateCursor = isServiciosRoute || isNosotrosRoute;
  ...
  const isFormElement = (element) => {
    ...
    return !!element.closest("input, textarea, select, button:not([data-cursor='view'])");
  };
  ...
  const isInteractiveElement = (element) => {
    ...
    return !!element.closest("a, button:not([data-cursor='view']), [role='button']");
  };
  ...
  const detectCursorState = (x, y) => {
    ...
    const isInSlider = element.closest(".infinite-slider-container");
    
    if (element.hasAttribute("data-cursor") && element.getAttribute("data-cursor") === "view") {
      return { 
        color, 
        showSvg: true, 
        useBlendMode: false,
  ...
  const restoreCirclesToBase = (x, y) => {
  ...
  useEffect(() => {
    ...
    // Crear contenedor en el portal
    const container = document.createElement('div');
    container.className = 'trail-cursor-portal';
    container.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 99999999;';
    ...
    targetCoordsRef.current.x = initialX;
    targetCoordsRef.current.y = initialY;
    currentCoordsRef.current.x = initialX;
    currentCoordsRef.current.y = initialY;

    if (cursorCoreRef.current) {
      cursorCoreRef.current.style.backgroundColor = currentColorRef.current;
    }

    // Handler de mousemove - SOLO usa clientX/clientY
    const handleMouseMove = (e) => {
      targetCoordsRef.current.x = e.clientX;
      targetCoordsRef.current.y = e.clientY;
    };

    // Animación con requestAnimationFrame y lerp
    const animate = () => {
      const lerpFactor = 0.15;
      
      currentCoordsRef.current.x = lerp(currentCoordsRef.current.x, targetCoordsRef.current.x, lerpFactor);
      currentCoordsRef.current.y = lerp(currentCoordsRef.current.y, targetCoordsRef.current.y, lerpFactor);
      
      const x = currentCoordsRef.current.x;
      const y = currentCoordsRef.current.y;

      const state = detectCursorState(x, y);

      if (state.showSvg && !showSvgRef.current) {
        setShowSvg(true);
      ...
      } else if (!state.showSvg && showSvgRef.current) {
        setShowSvg(false);
      ...
      const baseSize = 24;
      const coreSize = state.isInSlider ? baseSize + 20 : baseSize;
      const coreOffset = coreSize / 2;

      if (cursorCoreRef.current && modeRef.current === "slider") {
        cursorCoreRef.current.style.transform = `translate3d(${x - coreOffset}px, ${y - coreOffset}px, 0)`;
      ...
      if (modeRef.current === "normal") {
        ...
        circles.forEach((circle, index) => {
          ...
          trailX = lerp(trailX, x, speed);
          trailY = lerp(trailY, y, speed);

          circle.style.transform = `translate3d(${trailX - circleSize / 2}px, ${trailY - circleSize / 2}px, 0)`;
    ...
    <>
      ...
      {showSvg && (
        <div
          style={{
            position: "fixed",
            left: svgPosition.x - 66.5,
            top: svgPosition.y - 16.75,
            pointerEvents: "none",
            zIndex: 99999999,
  ...
  return containerRef.current ? createPortal(cursorContent, containerRef.current) : null;
};
