...
// --- Función para Errores (Log) ---
function errorLog($message, $data = null)
{
    $candidates = [
        __DIR__ . '/../../form-errors.log',
        __DIR__ . '/../form-errors.log',
        __DIR__ . '/form-errors.log'
    ];
    $logFile = null;
...
// --- Función para Guardar Leads (Log de Llenado) ---
function saveLeadToFile($formData)
{
    $candidates = [
        __DIR__ . '/../../form-leads.log',
        __DIR__ . '/../form-leads.log',
        __DIR__ . '/form-leads.log'
    ];
    $logFile = null;
    ...
    $timestamp = date('Y-m-d H:i:s');
    $entry = "[{$timestamp}] NUEVO LEAD | ORIGEN: " . ($formData['ORIGEN'] ?? 'N/A') . " | DATOS: " . json_encode($formData, JSON_UNESCAPED_UNICODE) . "\n";
    @file_put_contents($logFile, $entry, FILE_APPEND | LOCK_EX);
}

errorLog('SCRIPT_START_UNIFIED', ['ip' => $_SERVER['REMOTE_ADDR'] ?? 'unknown']);
...
try {
    ...
    if ($recaptchaSecret !== '' && !empty($recaptchaToken)) {
        if (!function_exists('curl_init')) {
            throw new \Exception('cURL no está habilitado en el servidor.');
        }
        ...
        if ($res) {
            ...
            if (!($vj['success'] ?? false)) {
                ...
                errorLog('RECAPTCHA_BLOCK', ['score' => $recaptchaScore]);
                echo json_encode(["success" => true, "message" => "Recibido"], JSON_UNESCAPED_UNICODE);
                exit;
            } elseif ($recaptchaScore < RECAPTCHA_LOW_CONFIDENCE_SCORE) {
                $isLowConfidence = true;
            }
        } else {
    ...
    errorLog('SMTP_RESULT', ['success' => $mailSent]);

    echo json_encode([
        "success" => true,
        "db_saved" => false, // Desactivado
        "file_saved" => true,
        "mail_sent" => $mailSent,
    ...
    errorLog('FATAL_EXCEPTION', $e->getMessage());
    // DEBUG TEMPORAL: Devolvemos el error real en la respuesta JSON para que puedas verlo en la consola
    echo json_encode([
        "success" => false,
